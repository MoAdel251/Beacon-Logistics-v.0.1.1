// ---------- Storage & Helpers ----------
    const Store = {
      key: 'lc_crm_v1',
      defaultData(){ return {
        users:[{id:'u_admin', username:'Mo', password:'mo1282', name:'Mohamed Adel'}],
        session:null,
        customers:[], shipments:[], drivers:[], invoices:[], clearances:[], deposits:[], driverDeposits:[]
      }},
      defaultData(){ return {
        users:[{id:'u_admin', username:'Ahmed', password:'Beacon2025', name:'Ahmed Makhlouf'}],
        session:null,
        customers:[], shipments:[], drivers:[], invoices:[], clearances:[], deposits:[], driverDeposits:[]
      }},
      defaultData(){ return {
        users:[{id:'u_admin', username:'MK', password:'Beacon2025', name:'Mahmoud Kotb'}],
        session:null,
        customers:[], shipments:[], drivers:[], invoices:[], clearances:[], deposits:[], driverDeposits:[]
      }},
      defaultData(){ return {
        users:[{id:'u_admin', username:'MH', password:'Beacon2025', name:'Mahmoud El Hadary'}],
        session:null,
        customers:[], shipments:[], drivers:[], invoices:[], clearances:[], deposits:[], driverDeposits:[]
      }},
    ...
    // ---------- Dashboard ----------
    function renderDashboard(root){ const csum = sumByCurrency(state.invoices, 'amount'); const clsum = sumByCurrency(state.clearances, 'cost'); const depositedSum = sumByCurrency(state.clearances, 'deposited'); const drvCost = sumByCurrency(state.drivers, 'cost'); const drvDeposited = sumByCurrency(state.drivers, 'deposited');
      root.innerHTML = `
    ...
    function deleteRecord(type, id){ if(!confirm('Delete this record?')) return; 
      if(type==='customer'){ state.customers = state.customers.filter(x=>x.id!==id); // also remove relations
    ...
    // ---------- Settings ----------
    function renderSettings(root){ root.innerHTML = `<div class="card"><h2>Settings</h2><div class="hint">Manage users & reset data</div><label>Users</label><div id="usersList" class="muted"></div><div style="display:flex; gap:8px; margin-top:8px"><button class="btn" onclick="addUserPrompt()">Add User</button><button class="btn" onclick="if(confirm('Clear all data? This cannot be undone.')){ state = Store.defaultData(); saveState(); }">Factory Reset</button></div></div>`; renderUsers(); }
